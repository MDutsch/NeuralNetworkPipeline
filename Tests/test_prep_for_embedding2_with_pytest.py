# Testbeschreibung:
# In den Trainingsdaten wurde "was_missing" als Marker für einen fehlenden Wert eingefügt
...
# Trainingsdaten
pd.DataFrame({
    'A': pd.Series(['zA', 'zB', 'zD', 'zA', 'zE', 'was_missing'],
    dtype=pd.CategoricalDtype(categories=['zB', 'zA', 'zC', 'zD', 'zE', 'was_missing'])),
...
# Testdaten
pd.DataFrame({
    'A': pd.Series(['zB', 'zB', 'zC', 'zE', 'was_missing', 'zG'],
    dtype=pd.CategoricalDtype(categories=['zC', 'zE', 'zB', 'zG', 'was_missing'])),
...
['unknown','was_missing','zA','zB','zD','zE'],
