...
#-----------------------------------------------------------------------------------------------------------------------
# Pipeline-Erstellung
TitanicPipeline = NeuralNetworkPipeline('Pipeline_with_Embeddings')
TitanicPipeline._set_model(KNN)
TitanicPipeline._set_trainalgo(optimizer='adam',
                                loss='binary_crossentropy',
                                metrics=['binary_accuracy'])
Stopper = EarlyStopping(min_delta=0.0001,
                        patience=80,
                        restore_best_weights=True)
TitanicPipeline._set_stopper(Stopper)
TitanicPipeline._set_preprocessing_options( maxcardhiddencat=15,
                                            maxcardcat=15,
                                            maxmisspercent=50,
                                            detection_excep=[],
...
TitanicPipeline._set_separate_inputs(separate_inputs={'Embedding1Input':'Name','Embedding2Input':'Ticket'})
#-----------------------------------------------------------------------------------------------------------------------
# Training
TrainValData = pd.read_csv(os.path.join("Datensaetze", "train.csv"))
TitanicPipeline._train(Data=TrainValData,
                       Target='Survived',
                       train_test_split_ratio=0.3,
                       batchsize=80,
                       epochs=500)
TitanicPipeline._show_trainchart()
#-----------------------------------------------------------------------------------------------------------------------
# Vorhersage
TestData = pd.read_csv(os.path.join("Datensaetze", "test.csv"))
prediction=TitanicPipeline._predict(TestData)
print('Vorhersage: \n', prediction)
np.savetxt("Vorhersage_mit_Embeddings.txt", prediction)
TitanicPipeline._save_pipeline_in_joblib()